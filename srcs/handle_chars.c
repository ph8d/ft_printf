...
/*   Updated: 2018/02/02 17:41:46 by rtarasen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		handle_str_wide(t_specs specs, va_list *arg_list)
{
	...
	while (str_wide[i] != '\0')
	{
		bytes = ft_count_active_bytes(str_wide[i]);
		if (specs.precision > 0 && specs.precision < bytes)	//	<--- weird statement, but it works
			break ;
		ft_uchar(str_wide[i]);
		str_len += bytes;
		specs.precision -= bytes;
		i++;
	}
